...
def update_cali():
    Session = sessionmaker(bind=engine)
    session = Session()
    query_cali = session.query(State).filter_by(name = 'California').first()
    query_cali.population = 50000000
    session.commit()
...
def delete_connecticut():
    Session = sessionmaker(bind=engine)
    session = Session()
    query_ct = session.query(State).filter_by(name = 'Connecticut').first()
    session.delete(query_ct)
    session.commit()
